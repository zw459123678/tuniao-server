 ...
 * @param {string} url - 要获取内容的URL
 * @returns {Promise<Array>} 包含组件信息的数组
 */
export async function extractNavigation(url) {
  try {
    // 获取远程HTML内容
    const response = await fetch(url);
    if (!response.ok) {
 ...
 * @returns {Promise<Object>} - 包含组件文档内容和元数据的对象
 */
export async function getComponentDocumentation(baseUrl, componentName) {
  try {
    ...
    console.error('Error getting component documentation:', error.message);
    throw error;
  }
}
